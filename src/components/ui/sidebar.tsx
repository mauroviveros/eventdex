...
const setSidebarStateCookie = (openState: boolean) => {
  if (!("cookieStore" in window)) return;

  const expires = new Date(Date.now() + SIDEBAR_COOKIE_MAX_AGE * 1000);
  const store = (window as Window & { cookieStore: SidebarCookieStore }).cookieStore;
  void store.set({
...
function SidebarSeparator({ className, ...props }: React.ComponentProps<typeof Separator>) {
  return (
    <Separator
      ...
      data-horizontal
      className={cn("mx-2 max-w-[calc(100%-16px)] bg-sidebar-border", className)}
      {...props}
    />
  );
}
